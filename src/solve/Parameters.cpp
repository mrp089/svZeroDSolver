...
void load_calibration_model(const nlohmann::json& config, Model& model) {
  ...
  for (auto& cn : connections) {
    auto ele1 = model.get_block(std::get<0>(cn));
    auto ele2 = model.get_block(std::get<1>(cn));
    if (ele1->get_vessel_type() == VesselType::inlet) {
      model.add_node({}, {ele1}, std::get<0>(cn) + ":" + ele1->get_name());
    } else if (ele2->get_vessel_type() == VesselType::outlet) {
      model.add_node({ele2}, {}, ele2->get_name() + ":" + std::get<1>(cn));
    } else {
